...
| 2026-08-23 | 결함수정 | 내비 덕킹 잔존 — 시스템 자동 덕킹 실측, GAIN+덕킹위임 무시 방식 전환 (docs/09 25절) → v0.9.5 |
...
## v0.10.0 기록 (2026-08-28~31 — 7·8차 개선)

- **알림 탭 → 앱+플레이어 열기**: setSessionActivity 미구현이 원인 — EXTRA_OPEN_PLAYER 경로 재사용.
- **영상 제스처 (유튜브 앱 패턴 통일)**: 싱글탭 → 중앙 반투명 ▶/⏸ 오버레이(아이콘 탭해야 동작, 재생 중 3초 자동 숨김·정지 시 유지) — 일반·전체화면 공통(전체화면 하단 재생버튼 중복 제거). 더블탭 좌/우 = ±10초(+플래시). PIP는 시스템 제약으로 제외.
- **채널 정보+댓글**: 조회수·좋아요·아바타·구독자 + 댓글 시트(보기 전용·더보기·HTML 정리). 채널 행 탭 → 유튜브 웹 채널 페이지. 설명은 태그 잔재로 제거(사용자).
- **네이티브 검색**: BigVideoCard(유튜브 앱식 세로 카드) + UTub 헤더 🔍 + **웹 돋보기 검색도 감지해 네이티브 결과로 자동 전환**(재감지 가드·웹뷰 홈 리셋 — back 동선 검증). 웹 검색 레이아웃 깨짐(§22 서버 결정)의 최종 해법.
- **라이브 타임시프트**: DVR 윈도우 있는 라이브에서 시크바·±10초·더블탭 활성 + "🔴 실시간"↔"실시간으로 ▶"(엣지 복귀). Lofi Girl 라이브 실검증.
- **서명 사고 → 영구 해결**: 타 PC 빌드와 서명 충돌 → 백업→재설치→복원(백업 기능 실전 검증) + **공용 keystore 저장소 포함**(어느 PC든 동일 서명).
- 신규: ChannelInfo.kt, BigVideoCard, keystore/utub-shared.jks. 테스트 75/75.
