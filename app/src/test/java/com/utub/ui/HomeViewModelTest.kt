...
class HomeViewModelTest {
    ...
    private val extractor = mockk<com.utub.extractor.StreamExtractor>(relaxed = true) {
        io.mockk.coEvery { trending() } returns emptyList()
        io.mockk.coEvery { search(any()) } returns emptyList()
    }
    ...
    fun detectsYouTubeLink() = runTest(dispatcher.scheduler) {
        val vm = HomeViewModel(recentPlayDao, extractor, settingsRepo(), stateHolder, connection)
    ...
    fun disabledDetectionIgnoresLink() = runTest(dispatcher.scheduler) {
        val vm = HomeViewModel(recentPlayDao, extractor, settingsRepo(clipboardDetect = false), stateHolder, connection)
        vm.onClipboardText("https://youtu.be/dQw4w9WgXcQ")
        dispatcher.scheduler.advanceUntilIdle()
        assertEquals(HomeViewModel.ClipboardState.None, vm.clipboard.value)
    }
